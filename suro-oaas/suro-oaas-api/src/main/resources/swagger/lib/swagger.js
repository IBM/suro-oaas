...
SwaggerApi.prototype.build = function() {
  var _this = this;
  if (this.index) {
	  fullUrl = this.url + this.index;
  }
  this.progress('fetching resource list: ' + fullUrl);
  var obj = {
...
var SwaggerResource = function(resourceObj, api) {
  ...
  if ((resourceObj.apis != null) && (this.api.resourcePath != null)) {
    ...
    if (this.path.substring(0, 4) === 'http') {
      ...
      // [CV] NOTE: we do have an issue here when composing the URL
      //            we need to check that there is only one / when
      //            attaching the two components.
      if ((this.api.basePath.lastIndexOf("/") == this.api.basePath.length - 1) && (this.path.indexOf("/") == 0)) {
    	  
    	  this.url = this.api.basePath + this.path.substring(1).replace('{format}', 'json');
      } else {
...
SwaggerResource.prototype.getAbsoluteBasePath = function(relativeBasePath) {
  ...
  // [CV] NOTE: this is a fix needed because the relative URLs get jammed with
  //            the base.
  //
  var urlLen = url.length;
  if (url.lastIndexOf("/") == urlLen-1) {
	  url = url.substring(0, urlLen - 2);
  }
