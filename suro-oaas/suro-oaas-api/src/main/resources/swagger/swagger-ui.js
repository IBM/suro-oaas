...
function program1(depth0,data) {
	...
	// [CV] NOTE: Removed the colon after name of the resource.
    //
	//
	// return " : ";
...
(function() {
  ...
  SwaggerUi = (function(_super) {
    ...
    SwaggerUi.prototype.initialize = function(options) {
      ...
      if (options.apiSelector == false) {
    	  
    	  $('form#api_selector').remove();
      }
    ...
    SwaggerUi.prototype.load = function() {
      ...
      if (url.indexOf("http") !== 0) {
    	// [CV} NOTE: ok in this case it means that this is a remote
    	//            address and we have to remove the index.html
    	//            file if there.
    	var location = window.location.href.toString();
    	var indexPos = location.indexOf('index.html');
    	if (indexPos !== -1) {
    		location = location.substr(0, indexPos);
    	}
    	// we do now have the location with the
    	// index stripped down.
    	
    	// now we sanitise, the URL incase there
    	// are references to '..' directories.
    	var dirs = url.split('/');
    	if (dirs.length) {
    		var remove = 0;
        	var clean = [];
        	...
        	// if remove is bigger than 0, that
        	// means that we have to clean the
        	// location from as many directory
        	// parts as the value of remove.
        	//
        	if (remove) {
        ...
        url = this.buildUrl(location, url);
      }
